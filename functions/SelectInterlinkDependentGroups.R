...
SelectInterLinkDependentGroups <- function(XLs_ori, FUSED=T, Inter2=T, Grouped=T){
#    XLs_ori <- XLs_ori_P; FUSED = T; Grouped=T; Inter2=T
  ...
  XLs1_ori_1 <- XLs_ori[XLs_ori$protein_a_cln != XLs_ori$protein_b_cln,] ## Interlinks
  XLs2_ori_2 <- XLs_ori[XLs_ori$protein_a_cln == XLs_ori$protein_b_cln,] ## Intralinks
  ...
  if (Grouped ==T & Inter2 ==F){
    Output  <- XLs1_ori_1[Ints1_Bin1,]
  }
  
  if (Grouped ==T & Inter2 ==T){
    Output  <- XLs1_ori_1[Ints1_Bin2,]
  }
